...
def mkranddir(place:str, deep:int, num:int) -> console_print:
...
def tree(path:str, avoid_sys_dirs:bool, level=1) -> console_print:
    ...
    if not path or type(path) is not str or not os.path.exists(path):
        print('Something went wrong, so I override path to default')
        if platform.system() == 'Linux':
            path = '/home/' + os.listdir('/home')[1]
    ...
    if avoid_sys_dirs == True:
        for item in [x for x in os.listdir(path) if not x.startswith('.') \
            ...
            if os.path.isdir(os.path.join(path,item)):
                ...
                if level > 1:
                    print('  '*(level-1), '|-', item, sep='')
                tree(os.path.join(path,item), True, level+1)
